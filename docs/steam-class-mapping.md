...
| `watchingbroadcast` | `.friend.watchingbroadcast` | Purple broadcast title. Valve provides no matching friend-row detail or hover rule, so plugin CSS supplies a default-Steam detail color and the neutral hover fallback described below. Themed detail text inherits the theme and uses reduced opacity for contrast. |
...
| `rcp-group` | Plugin-owned | Group conversations deliberately do not impersonate a persona state. Default Steam gets its group-list foreground palette; themes supply the inherited neutral foreground while plugin CSS keeps secondary-text contrast and the neutral hover fallback described below. |
...
Valve defines presence-specific friend-row hover backgrounds only for online, in-game, and offline rows. Broadcast, group, and unresolved rows therefore share `rgba(47,56,68,.5)`, the neutral hover used by Steam's own `.ChatRoomListGroupItem:hover` and `.ChatRoomListGroupItem_header:hover`. The complete selector is wrapped in `:where(...)`, giving it zero specificity: any native or theme rule that matches the row wins even though the plugin's main stylesheet is appended later.
...
Theme-hardening rules learned from live themes, encoded above: match the native tab's `box-sizing`, toggle its real active-state class, reset row width so forced margins inset rather than overflow, keep fallback hovers at zero specificity, avoid native hook classes whose themed sizing assumes native markup (`labelHolder`), scroll the card rather than the container, and pin only the Chats label's opacity against label-hiding designs.
