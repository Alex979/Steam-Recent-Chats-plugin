    ...
    div.rcp-list-content.listContentContainer.friendGroup
      div.rcp-row-wrapper[.unreadFriend]
        div.rcp-row.friend.friendStatusHover.(online|ingame|watchingbroadcast|offline)[.awayOrSnooze]
          ...
          span.rcp-copy.labelHolder.(online|ingame|watchingbroadcast|offline)[.awayOrSnooze]
...
| `labelHolder` + presence (`awayOrSnooze` when away) | `.friend .labelHolder`; theme selectors that require a nested presence/state holder | Every copy block recreates Steam's nested state markup so native/theme foreground and opacity rules match. Plugin CSS neutralizes the hook's single-line width, padding, and transform rules. |
| Resolved persona name/status tokens | Theme selectors such as `.online .<playerName>`, `.awayOrSnooze .<playerName>`, and the corresponding rich-presence label | Row text receives the same semantic theme colors as the native Friends list — several themes (SpaceTheme, Material) color names and subtext exclusively through these nested tokens, never through row-level classes. Only the two text tokens are reused; plugin CSS strips their mask, margin, and padding, and pins Recent Chats font size, weight, and line height with `!important`. |
...
| Broad `findClassModule` persona/avatar/Friends adoption | Removed. Avatar, container, and `LastMessage` tokens impose foreign geometry and colors. The narrow text resolver retains only `playerName` and `richPresenceLabel`, with their non-color box effects neutralized. |
...
Adopting a native class is an open-ended contract: the element then matches every current and future Steam and theme rule for that hook, and the plugin can only neutralize properties it has enumerated. This bit concretely with the resolved persona text tokens — Minimal Dark's "smaller font size" option ships `.<playerName> { font-size: 12px !important }` against the bare token, which shrank only away rows back when only they carried the token. Partial adoption has its own failure mode: SpaceTheme colors names and subtext exclusively through the nested tokens, so token-carrying rows followed the theme's palette while the rest fell back to Valve defaults. Every row therefore adopts the same nested structure — consistency over selective exposure. The geometry blocklists neutralize what live themes are known to set, and the persona text spans additionally pin the plugin's own typography with `!important` (equal-specificity `!important` ties resolve to the plugin sheet, which is appended after the theme link). A new theme styling an un-enumerated property on an adopted hook will leak through; the fix is another entry in the corresponding neutralization rule in `frontend/styles.ts`.

Theme-hardening rules learned from live themes, encoded above: match the native tab's `box-sizing`, toggle its real active-state class, reset row width so forced margins inset rather than overflow, express every visual row state through native presence structure, neutralize native hook geometry when a semantic theme selector requires that hook, scroll the card rather than the container, pin plugin typography on adopted text tokens, and pin only the Chats label's opacity against label-hiding designs.
