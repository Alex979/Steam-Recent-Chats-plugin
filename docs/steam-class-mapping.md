...
| Resolved persona name/status tokens | Theme selectors such as `.online .awayOrSnooze .<playerName>` and the corresponding rich-presence label | Away text receives the same semantic theme colors as the native Friends list. Only the two text tokens are reused; plugin CSS strips their mask, margin, and padding, and pins Recent Chats font size, weight, and line height with `!important`. |
...
Adopting a native class is an open-ended contract: the element then matches every current and future Steam and theme rule for that hook, and the plugin can only neutralize properties it has enumerated. This bit concretely with the resolved persona text tokens — Minimal Dark's "smaller font size" option ships `.<playerName> { font-size: 12px !important }` against the bare token, which shrank only away rows because only they carry the token. The geometry blocklists neutralize what live themes are known to set, and the persona text spans additionally pin the plugin's own typography with `!important` (equal-specificity `!important` ties resolve to the plugin sheet, which is appended after the theme link). A new theme styling an un-enumerated property on an adopted hook will leak through; the fix is another entry in the corresponding neutralization rule in `frontend/styles.ts`.
